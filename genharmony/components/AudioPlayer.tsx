  ...
  if (audioUrl) return (
    <div className="flex items-center gap-3 rounded-sm border border-line bg-rail/60 px-4 py-2.5">
      ...
      <button onClick={togglePlay}
        className="flex h-8 w-8 items-center justify-center rounded-full border border-pulse/50 text-pulse hover:bg-pulse/10">
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3 rounded-sm border border-line/60 bg-rail/40 px-4 py-2.5">
        ...
        <p className="flex-1 font-mono text-[11px] text-muted">No audio — generate with Suno/Udio and paste the URL</p>
      ...
      {showInput && (
        <div className="space-y-2">
          <Input label="Audio URL" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} placeholder="https://cdn.suno.ai/..." />
